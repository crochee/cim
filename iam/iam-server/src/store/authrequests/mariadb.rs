...
pub async fn create(pool: &MySqlPool, content: &Content) -> Result<ID> {
    ...
    let response_type =
        serde_json::to_string(&content.response_types).map_err(Code::any)?;
    let scope = serde_json::to_string(&content.scopes).map_err(Code::any)?;
...
pub async fn get(pool: &MySqlPool, id: &str) -> Result<AuthRequest> {
    ...
    Ok(AuthRequest {
        ...
        response_types: serde_json::from_str(&row.response_type)
            .map_err(Code::any)?,
        scopes: serde_json::from_str(&row.scope).map_err(Code::any)?,
        ...
        claims: match row.claims {
            Some(v) => serde_json::from_str(&v).map_err(Code::any)?,
            None => None,
        },
...
pub async fn update(
    ...
    let claims = serde_json::to_string(&opts.claims).map_err(Code::any)?;
    sqlx::query!(
