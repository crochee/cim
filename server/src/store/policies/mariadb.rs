...
pub async fn update(
    ...
    if let Some(statement) = &opts.statement {
        ...
        let content = serde_json::to_string(statement).map_err(errors::any)?;
        update_content
...
pub async fn get(
    ...
    let v = row.try_get::<u64, _>("user_id").map_err(errors::any)?;
...
pub async fn get_by_user(
    ...
    let user_id: u64 = user_id_str.parse().map_err(errors::any)?;
    let sql = format!(
